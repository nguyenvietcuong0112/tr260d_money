...
public class BudgetManager {
    ...
    // Phương thức cập nhật chi phí của một com.budgettracker.personalfinance.trackmoney.smartbudget
    public void updateBudgetExpenses(String budgetName) {
        // Lấy danh sách các giao dịch từ SharePreferenceUtils
        List<TransactionModel> transactions = sharePreferenceUtils.getTransactionList();

        // Tính toán tổng chi tiêu cho com.budgettracker.personalfinance.trackmoney.smartbudget cụ thể
        double totalExpenses = 0.0;
        for (TransactionModel transaction : transactions) {
            if ("Expense".equals(transaction.getTransactionType()) && budgetName.equals(transaction.getBudget())) {
                totalExpenses += Double.parseDouble(transaction.getAmount());
        ...
        // Cập nhật spentAmount của com.budgettracker.personalfinance.trackmoney.smartbudget tương ứng
        ...
        // Lưu lại danh sách com.budgettracker.personalfinance.trackmoney.smartbudget đã cập nhật
