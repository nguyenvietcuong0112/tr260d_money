...
public class SharePreferenceUtils {
    ...
    public static int getAppLaunchCount(Context context) {
        SharedPreferences pref = context.getSharedPreferences("data", Context.MODE_PRIVATE);
        return pref.getInt(KEY_APP_LAUNCH_COUNT, 0);  // Mặc định là 0 nếu chưa có giá trị
    }

    // Phương thức lưu số lần mở ứng dụng vào SharedPreferences
    public static void setAppLaunchCount(Context context, int count) {
        SharedPreferences pref = context.getSharedPreferences("data", Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = pref.edit();
        editor.putInt(KEY_APP_LAUNCH_COUNT, count);
        editor.apply();
    }
}
